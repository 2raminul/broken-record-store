...
export class OrdersController {
  ...
  create(@Body() dto: CreateOrderInDto): Promise<OrderOutDto> {
    return this.ordersService.create(dto) as unknown as Promise<OrderOutDto>;
  }
  ...
  @ApiDefaultErrorResponses()
  findAll(@Query() query: FindOrdersInDto): Promise<PaginatedOutDto<OrderOutDto>> {
  ...
  findOne(@Param('id', ParseObjectIdPipe) id: string): Promise<OrderOutDto> {
    return this.ordersService.findOne(id) as unknown as Promise<OrderOutDto>;
  }
}
