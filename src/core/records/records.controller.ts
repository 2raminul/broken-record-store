...
export class RecordsController {
  ...
  @ApiResponse({ status: 201, description: 'Record created', type: RecordOutDto })
  @ApiDefaultErrorResponses()
  create(@Body() dto: CreateRecordInDto): Promise<RecordOutDto> {
    return this.recordsService.create(dto) as unknown as Promise<RecordOutDto>;
  }

  @Put(':id')
  @ApiOperation({ summary: 'Update a record' })
  @ApiResponse({ status: 200, description: 'Record updated', type: RecordOutDto })
  @ApiDefaultErrorResponses()
  update(
    @Param('id', ParseObjectIdPipe) id: string,
    @Body() dto: UpdateRecordInDto,
  ): Promise<RecordOutDto> {
    return this.recordsService.update(id, dto) as unknown as Promise<RecordOutDto>;
  }

  @Get()
  @ApiOperation({ summary: 'List records with optional filters and pagination' })
  @ApiResponse({ status: 200, description: 'Paginated list', type: PaginatedOutDto })
  @ApiDefaultErrorResponses()
  findAll(@Query() query: FindRecordsInDto): Promise<PaginatedOutDto<RecordOutDto>> {
  ...
  findOne(@Param('id', ParseObjectIdPipe) id: string): Promise<RecordOutDto> {
    return this.recordsService.findOne(id) as unknown as Promise<RecordOutDto>;
  }
}
